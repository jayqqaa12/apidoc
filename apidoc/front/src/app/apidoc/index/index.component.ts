...
export class IndexComponent implements OnInit {
  ...
  getBaseUrl(){
    console.log('root url = '+this.baseUrl)

    this.http.get(apis.baseParam).subscribe(
      data => {
        if (data) {
          this.baseUrl= data['url']
          const map =data['header']
          this.baseHeader = new HttpHeaders(map)
        }
      }
    );
  ...
  removeAction(){
    this.http.delete(apis.removeAction+"/"+this.action.id).subscribe(
      data => {
        //刷新当前页面
         this.getDetailClick(this.action)
      }
    );
  }
  ...
  buildRequestParams(action) {
    if (action) {
      this.mockUrl = this.baseUrl + action.mapping;//演示的url默认为后台返回的mapping
      this.mockMethod = action.requestMethod;//演示的请求方式
      this.demoUrl = this.baseUrl + action.mapping;//示例的url
      ...
      if (this.contain(requestType, "URL")) {
        if (action.requestParam && action.requestParam.params && action.requestParam.params.length > 0) {//存在参数
          ...
          for (const param of  action.requestParam.params) {
            i++;
            if (i === 1) {
              this.mockUrl = this.baseUrl + action.mapping + "?" + param.name + "=" + this.getDefaultValue(param);
              this.demoUrl = this.baseUrl + action.mapping + "?" + param.name + "=" + (param.description ? param.description : "参数");
            } else {
          ...
          this.mockUrl = this.baseUrl + action.mapping;
          this.demoUrl = this.baseUrl + action.mapping;
        }
      ...
      //req file
      if (this.contain(requestType, "BLOB")) {
        this.mockShowFile = true;
        if (action.requestParam && action.requestParam.params && action.requestParam.params.length > 0) {
  ...
  sendTest() {
    ...
    // } else {//发送测试
      ...
      if (this.contain(this.mockMethod, 'GET')) {
        this.http.get(this.mockUrl,{headers:this.baseHeader}).subscribe(data => this.success(data), error => this.error4sendTest(error));
      } else if (this.contain(this.mockMethod, 'POST')) {
        this.http.post(this.mockUrl, this.mockReqParams,{headers:this.baseHeader}).subscribe(data => this.success(data), error => this.error4sendTest(error));
      } else if (this.contain(this.mockMethod, 'PUT')) {
        this.http.put(this.mockUrl, this.mockReqParams,{headers:this.baseHeader}).subscribe(data => this.success(data), error => this.error4sendTest(error));
      } else if (this.contain(this.mockMethod, 'DELETE')) {
        this.http.delete(this.mockUrl,{headers:this.baseHeader}).subscribe(data => this.success(data), error => this.error4sendTest(error));
      }
    // }
...
//编辑详情的 请求参数
  ...
  private addParam(isReturn, pid) {
    ...
    if(isReturn){
      this.detail.responseParam.params ? true : this.detail.responseParam.params = [];
      this.detail.responseParam.params.push(param);
    }else{
