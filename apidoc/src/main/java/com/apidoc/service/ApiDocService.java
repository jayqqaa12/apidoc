...
import com.apidoc.utis.ParseUtil;
 ...
 */
@Service
@ConditionalOnProperty(value = "config.apidoc", havingValue = "true")
public class ApiDocService {
    ...
    private void createTable() {
        ...
        String createInfo = "CREATE TABLE `apidoc_info` (\n" +
                ...
                "\t`package_name` VARCHAR(200) NOT NULL DEFAULT '' COMMENT '包名，用于区别一个项目中的多个文档' COLLATE 'utf8_bin',\n" +
                "\tPRIMARY KEY (`id`),\n" +
                "\tUNIQUE INDEX `title_package_name` (`title`, `package_name`)\n" +
                ")\n" +
                "COMMENT='文件基本信息'\n" +
                "COLLATE='utf8_bin'\n" +
                "ENGINE=InnoDB\n" +
        ...
        String createModule = "CREATE TABLE `apidoc_module` (\n" +
                ...
                "\t`package_name` VARCHAR(200) NOT NULL COMMENT '包名，区分不用的文档' COLLATE 'utf8_bin',\n" +
                "\t`class_list` VARCHAR(1000) NOT NULL COMMENT '类全名，多个之间用英文逗号隔开' COLLATE 'utf8_bin',\n" +
                "\tPRIMARY KEY (`id`),\n" +
                "\tUNIQUE INDEX `name_package_name` (`name`, `package_name`)\n" +
                ")\n" +
                "COMMENT='文档模块信息'\n" +
                "COLLATE='utf8_bin'\n" +
                "ENGINE=InnoDB\n" +
        ...
        String createAction = "CREATE TABLE `apidoc_action` (\n" +
                ...
                "\t`request_description` TEXT NULL COMMENT '请求参数描述' COLLATE 'utf8_bin',\n" +
                "\t`response_description` TEXT NULL COMMENT '响应参数描述' COLLATE 'utf8_bin',\n" +
                "\tPRIMARY KEY (`id`),\n" +
                "\tUNIQUE INDEX `moduleId_methoduuid` (`module_id`, `methoduuid`)\n" +
                ")\n" +
                "COMMENT='文档接口信息'\n" +
                "COLLATE='utf8_bin'\n" +
                "ENGINE=InnoDB\n" +
        ...
        String createParam = "CREATE TABLE `apidoc_param` (\n" +
                ...
                "\t`data_type` VARCHAR(50) NULL DEFAULT NULL COMMENT '数据类型' COLLATE 'utf8_bin',\n" +
                "\t`description` VARCHAR(500) NULL DEFAULT NULL COMMENT '描述' COLLATE 'utf8_bin',\n" +
                "\t`default_value` VARCHAR(500) NULL DEFAULT NULL COMMENT '默认值' COLLATE 'utf8_bin',\n" +
                "\t`required` TINYINT(4) NOT NULL DEFAULT '1' COMMENT '是否必须',\n" +
                "\t`action_id` INT(11) NOT NULL COMMENT '接口id',\n" +
                "\t`returnd` TINYINT(4) NOT NULL DEFAULT '0' COMMENT '是否是返回值',\n" +
                "\t`pid` INT(11) NOT NULL DEFAULT '0' COMMENT '父参数id',\n" +
                "\t`pclass_name` VARCHAR(200) NOT NULL DEFAULT '0' COMMENT '父类名  所属类名' COLLATE 'utf8_bin',\n" +
                "\tPRIMARY KEY (`id`),\n" +
                "\tINDEX `FK_apidoc_param_apidoc_action` (`action_id`)\n" +
                ")\n" +
                "COMMENT='文档参数信息'\n" +
                "COLLATE='utf8_bin'\n" +
                "ENGINE=InnoDB\n" +
    ...
    private boolean existTable(String param) {
        String sql = "SELECT count(*) FROM " + param;
     ...
     * @return String 基本信息
     */
    public ApidocInfo getInfo(String package_name) {
        ApidocInfo apidocInfo = apidocInfoDao.selectByPackageName(package_name);
        if (apidocInfo != null) {
            return apidocInfo;
        } else {
     ...
     */
    @Transactional
    public List<ApidocModule> getModules(String package_name) {
        //查询数据库所有该package下的模块信息
        Set<ApidocModule> modules4db = apidocModuleDao.selectByPackageName(package_name);
        ...
        //扫描包得到类
        Set<Class> classSet = ClassScanUtil.getClass4Annotation(package_name, Api.class);
        if (classSet.isEmpty()) {
            throw new RuntimeException("该包下没有class文件： " + package_name);
        } else {
            for (Class claszz : classSet) {
                //获取元数据
                Api api = (Api) claszz.getAnnotation(Api.class);
                String name = ParseUtil.isNotEmpty(api.value()) ? api.value() : claszz.getName();//没有写明模块名称时 默认取类名全称 com.xxx.xxx
                Integer seq = Integer.MAX_VALUE;//默认排序int最大值
                ...
                if (apidocModule != null) {//模块已存在时，判断class是否已经存在模块信息中
                    ...
                    apidocModule.setSeq(seq);
                    apidocModule.setPackageName(package_name);
    ...
    public List<ApidocAction> getActions(Integer moduleId) {
        ...
        if (ParseUtil.isEmpty(classListStr)) {
            return null;
        }
        ...
        if (classList.length > 0) {
            ...
            for (int i = 0; i < classList.length; i++) {
                ...
                Map<String, Map<String, String>> methodNotes = ParseUtil.getMethodNotes(className);
                ...
                if (claszz != null) {
                    ...
                    if (methodList.size() > 0) {
                        ...
                        for (Method method : methodList) {
                            ...
                            //找不到对应的mapping 说明不是接口
                            if (!SpringUtil.isMapping(method)) {
                                continue;
                            }
                            ...
                            if (action == null) {
                                ...
                                if (methodNotes.get(methoduuid) != null) {
                                    action.setDescription(methodNotes.get(methoduuid).get("desc"));
                                    action.setName(methodNotes.get(methoduuid).get("name"));
                                }
                                //有注释时用注释 没有时默认方法的名称
                                action.setName(ParseUtil.isEmpty(action.getName()) ? method.getName() : action.getName());
                                action.setSeq(Integer.MAX_VALUE);
                            ...
                            if (methodNotes.get(methoduuid) != null)
                                ParseUtil.parseParams(action.getId(), methodNotes.get(methoduuid).get("params"));
            ...
            //排序 默认按名称排序 存在seq时,seq优先排序
            actions4front.sort(Comparator.comparing(ApidocAction::getName));
            actions4front.sort(Comparator.comparing(ApidocAction::getSeq));
    ...
    public Detail getDetail(Integer id, String methoduuid) {
        ...
        if (split.length == 2) {
            ...
            if (claszz != null) {
                ...
                if (method != null) {
                    String basemapping = SpringUtil.getMapping(claszz);
                    String methodMapping = SpringUtil.getMapping(method);

                    if (basemapping != null && !basemapping.startsWith("/")) basemapping = "/" + basemapping;
                    if (methodMapping != null && !methodMapping.startsWith("/")) methodMapping = "/" + methodMapping;

                    if (basemapping == null) basemapping = "";
                    if (methodMapping == null) methodMapping = "";

                    String mapping = basemapping + methodMapping; //url 映射 mapping 为类上的mapping+方法上的mapping
    ...
    private Params getParams(Integer id, Method method) {
        ...
        params.setParams(list2Tree(apidocParams));
        return params;
    }
    ...
    private List<ApidocParam> generateParams(Method method, Integer actionId) {
        ...
        for (int i = 0; i < parameterTypes.length; i++) {
            ...
            isType(list, actionId, paramNames[i], parameterTypes[i],
                    genericParameterTypes[i], null, 0, true, false);
        }
        return list;
    }
    ...
    private synchronized void isType(List<ApidocParam> list, Integer actionId, String paramName, Class tclass, Type genType,
        ...
        item.setPid(pid);
        if (ParseUtil.isEmpty(paramName)) {//基本类型可能取不到class的名称 这里toString一下
            paramName = tclass.toString();
        }
        item.setName(paramName);
        item.setDefaultValue(getObjectDefaultValue(tclass) + "");
        item.setRequired(false);//默认可选


        if (tclass.getAnnotation(NotNull.class) != null) {
        ...
        if (pclass != null) {//设置所属类名
            ...
            Map<String, String> fieldsNotes = ParseUtil.getFieldsComment(pclass);

            if (fieldsNotes != null) {
                String description = fieldsNotes.get(paramName);
                if (ParseUtil.isNotEmpty(description)) {
            ...
            String desc = ParseUtil.getFieldsNotes(actionId, paramName);
            item.setDescription(StringUtils.isEmpty(desc) ? paramName : desc);
            item.setPclassName("0");
        }
        item.insert();
        ...
        if (genType instanceof ParameterizedType) {
            ...
            } else if (IPage.class.isAssignableFrom(tclass)) {// 针对自定义类型 IPage<T> 特殊处理
                //保存对象中属性名为data的类型
                class4data = (Class) parameterArgTypes[0];
            }
        ...
        //字符串
        else if (CharSequence.class.isAssignableFrom(tclass) || Character.class.isAssignableFrom(tclass) || Const.string.equals(typeMap.get(tclass))) {
            item.setDataType(Const.string);
            item.updateById();
            list.add(item);
        }
        //boolean
        else if (Boolean.class.isAssignableFrom(tclass) || Const.booleann.equals(typeMap.get(tclass))) {
            item.setDataType(Const.booleann);
            item.updateById();
            list.add(item);
        }
        //文件 MultipartFile
        else if (InputStreamSource.class.isAssignableFrom(tclass)) {
        ...
        // 如果是枚举类型
            ...
            item.setDataType(Const.number);
            //解析enum类的所有 属性生成描述
            item.setDescription(getEnumDesc(item.getDescription(), tclass));
            item.setDefaultValue(null);
            item.updateById();
            list.add(item);
        }
        //自定义对象类型
        else if (isMyClass(tclass)) {
            item.setDataType(Const.object);//自定义对象类型为对象的名称
            item.updateById();
            list.add(item);
    ...
    private String getEnumDesc(String description, Class tclass) {
...
//        }
        ...
        switch (requestMethod) {
            ...
            case Const.POST:
                ...
                if (containsRequestBodyAnnotation(method.getParameterAnnotations())) {
                    ...
                    if (containsFile(method.getParameterTypes()))
                        type = Const.BLOB;
                    else
                        type = Const.FROM;
                }
    ...
    private boolean containsFile(Class[] parameterAnnotations) {
        for (Class classes : parameterAnnotations) {
            if (MultipartFile.class.isAssignableFrom(classes) ||
                    MultipartFile[].class.isAssignableFrom(classes)) {
                return true;
            }
        }
        return false;
    ...
    public boolean deleteActions(Integer id) {
        //其实是删除接口下面所以的参数
        return apidocParamDao.deleteByActionId(id) > 0;
    }
}
